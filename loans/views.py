...
def create(self, request, *args, **kwargs):
    ...
    try:
        data = request.data
        print("=" * 50)
        print("LOAN CREATION REQUEST")
        print("Request data:", data)
        print("User:", request.user)
        print("User ID:", request.user.id if request.user else None)
        print("=" * 50)
        ...
        # Get user (the logged-in user who is creating the loan)
        user = request.user
        
        # IMPORTANT: Check if user exists
        if not user or not user.id:
            return Response({
                'error': 'You must be logged in to create a loan'
            }, status=status.HTTP_401_UNAUTHORIZED)
        
        # Get branch from user (if user has one)
        branch = user.branch if hasattr(user, 'branch') and user.branch else None
        ...
        loan = Loan(
            ...
            status='draft',
            created_by=user,  # THIS IS CRITICAL - set the user
